...
class _MyActivityState extends State<MyActivity>
  ...
  void _getActivityList() async {
    activitys =
        await _activityService.getActivityListByUser(0, widget.user.uid);
    ;
...
class _ActivityWidgetState extends State<ActivityWidget> {
  ...
  initState() {
    if (Global.profile.user != null) {
      _imHelper.selActivityState(this._activity.actid, Global.profile.user!.uid,
          (List<String> actid) {
        if (actid.length > 0) {
          setState(() {
            retLike = true;
