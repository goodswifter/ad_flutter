  ...
  Widget build(BuildContext context) {
            ...
            child: Column(
              ...
              children: <Widget>[
                Row(
                  ...
                  children: <Widget>[
                    Text('全部留言(${_listComments.length.toString()})',
                        style: TextStyle(
                            color: Colors.black87,
                            fontSize: 14,
                            fontWeight: FontWeight.bold)),
    ...
    return Scaffold(
      ...
      body: _activity != null
                ...
                builder: (BuildContext context, LoadStatus? mode) {
                  ...
                  if (mode == LoadStatus.idle) {
                    ...
                    body = Center(
                      child: CircularProgressIndicator(
                        valueColor:
                            AlwaysStoppedAnimation(Global.profile.backColor),
                      ),
                    );
          ...
          : promptWidget,
      bottomNavigationBar:
          _activity != null ? buildBottomButton() : SizedBox.shrink(),
    );
  }

  Future<void> getActivityInfo() async {
    _activity =
        await _activityService.getActivityInfo(widget.actid, errorCallBack);
    if (_activity == null) {
      promptWidget = Center(
        child: Text('活动已经被删除了'),
      );
    ...
    if (Global.profile.user == null) {
      _listComments =
          await _activityService.getCommentList(widget.actid, 0, errorCallBack);
    } else {
      ...
      Map likecollectionstate = await _activityService.getLikeCollectionState(
          widget.actid, Global.profile.user!.uid);
      ...
      _listComments = await _activityService.getCommentList(
          widget.actid, Global.profile.user!.uid, errorCallBack);
    }
    if (mounted) setState(() {});
  ...
  Column buildActivityImg() {
    ...
    if (_listimgs.length > 0) {
      for (int i = 0; i < _listimgs.length; i++) {
        imglist.add(
            {"tag": UniqueKey().toString(), "img": _listimgs[i].toString()});
      }
    }
    double initheigth = 0;
...
//                ),
  ...
  Future<void> updateLike() async {
    bool ret = await _activityService.updateLike(_activity!.actid,
        Global.profile.user!.uid, Global.profile.user!.token!, errorCallBack);
    if (ret) {
  ...
  Future<void> updateDelLike() async {
    bool ret = await _activityService.delLike(_activity!.actid,
        Global.profile.user!.uid, Global.profile.user!.token!, errorCallBack);
    if (ret) {
      _activity!.likenum = _activity!.likenum - 1;
      setState(() {
  ...
  Future<void> updateCollection() async {
    bool ret = await _activityService.updateCollection(_activity!,
        Global.profile.user!.uid, Global.profile.user!.token!, errorCallBack);
    if (ret) {
  ...
  Future<void> updateDelCollection() async {
    bool ret = await _activityService.delCollection(_activity!.actid,
        Global.profile.user!.uid, Global.profile.user!.token!, errorCallBack);
    if (ret) {
      _activity!.collectionnum = _activity!.collectionnum - 1;
      setState(() {
  ...
  Future<void> joinActivity() async {
    ...
    GroupRelation? groupRelation = await imhelper.getGroupRelationByGroupid(
        Global.profile.user!.uid, _activity!.actid);
    ...
    if (groupRelation != null) {
      ...
      if (ret > 0) {
        Navigator.pushNamed(context, '/MyMessage', arguments: {
  ...
  Future<void> updatedelMessageReply(int replyid) async {
    ...
    if (ret) {
      _listComments = await _activityService.getCommentList(
          _activity!.actid, Global.profile.user!.uid, errorCallBack);
      sortComment();
    }
  }

  Future<void> updateDelMessage(int commentid) async {
    bool ret = await _activityService.delMessage(Global.profile.user!.token!,
        Global.profile.user!.uid, commentid, _activity!.actid, errorCallBack);
    if (ret) {
      _listComments = await _activityService.getCommentList(
          _activity!.actid, Global.profile.user!.uid, errorCallBack);
      sortComment();
  ...
  updateActivityTime() async {
    await _activityService.updateActivityTime(_activity!.actid,
        Global.profile.user!.uid, Global.profile.user!.token!, errorCallBack);
  }

  Future<void> sendToMessage(int commentid, int touid, String content,
      {User? touser, String captchaVerification = ""}) async {
    ...
    if (commentid == 0) {
      ...
      if (commentid > 0) {
        _listComments.insert(
            0,
            Comment(commentid, _activity!.actid, Global.profile.user!, content,
                0, CommonUtil.getTime(), 0));
        sortComment();
      }
    } else {
  ...
  List<Widget> buildMemberList() {
    ...
    List<User> members = [];
    count =
        _activity!.members!.length > count ? count : _activity!.members!.length;
    ...
    if (_activity!.members!.length > count) {
      widgets.add(Padding(
        ...
        child: InkWell(
          ...
          onTap: () {
            Navigator.pushNamed(context, '/ActivityMember',
    ...
    return GestureDetector(
      onTap: () {
        Navigator.pushNamed(context, '/ActivityInfo',
            arguments: {"actid": activity.actid}).then((val) {});
      },
      child: Card(
        elevation: 0,
        child: Column(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Expanded(
                      child: Container(
                    alignment: Alignment.centerRight,
                    child: CommonUtil.getTextDistance(activity.lat,
                        activity.lng, Global.profile.lat, Global.profile.lng),
                  )),
                ],
              ),
            ),
    ...
    return showDialog<Null>(
        ...
        builder: (BuildContext context) {
          return new AlertDialog(
            title: new Text('结束活动后将不在显示，确定要结束吗?',
    ...
    showDialog<Null>(
      ...
      builder: (_) {
        return BlockPuzzleCaptchaPage(
          onSuccess: (v) {
            sendToMessage(blockcommentid, blocktouid, _message,
