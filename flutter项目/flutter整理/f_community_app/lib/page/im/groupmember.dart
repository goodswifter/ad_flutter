...
class _State extends State<GroupMember> {
  ...
  getAllUsers() async {
    activity = await _activityService.getActivityMember(
        widget.timeline_id, errorCallBack);
    if (activity != null) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title:
              Text('群成员', style: TextStyle(color: Colors.black, fontSize: 16)),
          centerTitle: true,
        ),
    ...
    return Container(
      ...
      child: ListTile(
        ...
        title: InkWell(
          ...
          onTap: () {
            if (widget.status == null || widget.status == 1) {
              Navigator.pushNamed(context, '/ManageActivityMember', arguments: {
                ...
                onPressed: () async {
                  ...
                  if (ret) {
                    int ret = await _imHelper.delGroupRelation(
                        widget.timeline_id, Global.profile.user!.uid);
                    if (Global.isInDebugMode) print(ret);
                    Navigator.pop(context);
                    Navigator.pop(context);
                    Navigator.pop(context);
                  } else {
