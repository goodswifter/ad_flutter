...
class _MyMessageState extends State<MyMessage> with TickerProviderStateMixin {
  ...
  AliyunService _aliyunService = AliyunService();
          ...
          if (_locationResult != null) {
            if (result["longitude"] != "" && result["adCode"] != "") {
              Navigator.pushNamed(context, '/MapLocationPicker', arguments: {
                ...
                "citycode":
                    CommonUtil.getCityNameByGaoDe(result["adCode"].toString()),
                "isMapImage": true
              }).then((dynamic value) {
                if (value != null) {
                  if (value["image"] == null) {
  ...
  Future<void> startPlayer(String filepath,
      {TimeLineSync? timeLineSync}) async {
    try {
  ...
  _getImMsgInit() async {
    timeLineSyncs = await _imHelper.getTimeLineSync(
        Global.profile.user!.uid, 0, 30, _groupRelation.timeline_id);
  ...
  Future<void> sendMessage(String content, int type, String localmsg,
      String localpath, String captchaVerification) async {
    ...
    if (serviceData != null && serviceData != "") {
      ...
      temLine.add(TimeLineSync(_groupRelation.timeline_id, 0, user.uid, time,
          user, this._localmsgcontent, this._contenttype, localpath, sourceid));
    }
    if (time != null && time != "") {
  ...
  _sendShared() async {
    ...
    if (!isQuit) {
      await sendMessage(
          widget.sharedcontent, 5, widget.localsharedcontent, "", "");
    }
  }

  _sendMsg() async {
    await sendMessage(
        _textEditingController.text, 0, _textEditingController.text, "", "");
    setState(() {
  ...
  _sendLocal(String imgFile, String imgwh, String address, String title,
      String longitude, String latitude, String localurl) async {
    String imgurl = imgFile;
    String msg =
  ...
  _sendDrawRedPacket(String content, String msg) async {
    ...
    List<TimeLineSync> temLine = [];
    temLine.add(TimeLineSync(_groupRelation.timeline_id, 0, 0, sendtime,
        Global.profile.user!, msg, 0, '', ""));
  ...
  _getGroupMember() async {
    if (_groupRelation.relationtype == 0 || _groupRelation.relationtype == 3) {
      Activity? activity = await _activityService.getActivityMember(
          _groupRelation.timeline_id, errorCallBack);
      if (activity != null) {
        if (activity.members != null && activity.members!.length > 0) {
          _imHelper.saveGroupMemberRelation(
              activity.members!, _groupRelation.timeline_id);
        }
      }
    }
    ...
    _imHelper.updateGroupRelation(
        _groupRelation.memberupdatetime!, _groupRelation.timeline_id);
  }
  ...
  _initMember() async {
    if (_groupRelation.memberupdatetime != null &&
      ...
      List<User>? users =
          await _imHelper.getGroupMemberRelation(_groupRelation.timeline_id);
              ...
              Expanded(
                ...
                onTap: () {
                  Navigator.pushNamed(context, '/GoodPriceInfo',
                      arguments: {"goodprice": _goodprice}).then((value) {
                    FocusScope.of(context).requestFocus(FocusNode());
                  });
                },
              )),
            ...
            _groupRelation.relationtype != 2
                ? IconButton(
                    ...
                    onPressed: () {
                      ...
                      Navigator.pushNamed(context, navurl, arguments: argMap)
                          .then((value) {
                        if (value != null) {
                          if (value == 2) {
                            _blockUser(2);
            ...
            if (state is NewMessageState) {
              ...
              for (GroupRelation tem in state.groupRelations) {
                if (tem.timeline_id == _groupRelation.timeline_id) {
                  _imBloc.add(Already(
                      Global.profile.user!, _groupRelation.timeline_id));
...
//                        ),
  ...
  Widget buildSysMsg(TimeLineSync timeLineSync) {
    if (timeLineSync.content!.indexOf("@安全活动规范@") >= 0) {
            ...
            child: Container(
              child: Column(
                ...
                children: [
                  ...
                  Text('为了确保您的资金安全，请遵守出来玩吧交易规范，一定要在平台内完成支付。',
      ...
      String temNotice =
          timeLineSync.content!.replaceAll("|sysactivitynotice:", '');
      return Container(
        margin: EdgeInsets.all(10),
        padding: EdgeInsets.all(10),
        decoration: new BoxDecoration(
  ...
  Widget buildMyContent(TimeLineSync timeLineSync) {
    ...
    if (timeLineSync.contenttype == ContentType.Type_Sound.index) {
      String key =
          timeLineSync.content!.replaceAll('|sound: ', '').replaceAll('|', '');
      ...
      widContent = Container(
        ...
        child: Row(
            ...
            mainAxisAlignment: MainAxisAlignment.start,
            children: <Widget>[
              ExtendedText(timeLineSync.content!,
      ...
      //组件有问题要在外面处理，里面也处理了一次
      String key =
          timeLineSync.content!.replaceAll('|img: ', '').replaceAll('|', '');
      ...
      widContent = Container(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.end, children: <Widget>[
          InkWell(
            ...
            child: ExtendedText(timeLineSync.content!,
                style: TextStyle(color: Colors.white),
                overflow: TextOverflow.ellipsis,
                maxLines: 50,
                specialTextSpanBuilder: MySpecialTextSpanBuilder(
                    showAtBackground: false, pagewidth: _pageWidth_2)),
          )
        ]),
      );
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.end, children: <Widget>[
          InkWell(
            onTap: () {
              ...
              if (sharedtype == "2") {
                Navigator.pushNamed(context, '/MomentInfo',
              ...
              if (redpacketid != null) {
                ...
                if (redpacket.touid != null && redpacket.touid > 0) {
                  ...
                  _imHelper.updateReceiveRedPacket(
                      timeLineSync.content!, timeLineSync.timeline_id!);
          ...
          onValueChanged: (int value) {
            ...
            if (value == 2) {
              ...
              if (widget.groupRelation.relationtype == 0 ||
                ...
                Navigator.pushNamed(context, '/ReportAllMessage', arguments: {
                  "sourcetype": 4,
      ...
      children: <Widget>[
        Container(
          ...
          child: NoCacheCircleHeadImage(
              imageUrl: timeLineSync.serderpicture!, uid: timeLineSync.sender!),
        ),
        Expanded(
          flex: 8,
          child: Container(
              ...
              child: InkWell(
                child: Container(
                    ...
                    child: Padding(
                        padding: EdgeInsets.all(10), child: widContent)),
                onTap: () async {
                  if (timeLineSync.isplay!) {
                    stopPlayer();
                  } else {
...
//                      FocusScope.of(context).requestFocus(_focusNode);
      ...
      children: [
        ...
        GestureDetector(
          ...
          onTap: () async {
            ...
            Navigator.pushNamed(context, '/RedPacket', arguments: {
              "timeline_id": _groupRelation.timeline_id,
  ...
  List<Widget> buildRedInfo(
      RedPacketModel redPacketModel, String content, String timeline_id) {
    List<Widget> redInfo = [];
    ...
    if (redPacketModel.isexpire) {
      ...
      redInfo.add(GestureDetector(
        child: Container(
          margin: EdgeInsets.only(top: 30),
          alignment: Alignment.center,
          child: Image(
            image: AssetImage(
              "images/openredpacket.png",
            ),
            fit: BoxFit.cover,
            width: 59,
            height: 59,
  ...
  Future<void> fileSave(TimeLineSync timeLineSync) async {
    ...
    Directory soundsDirectory =
        await new Directory('${directory.path}/im/sounds/')
...
//      await audioPlayer.play(localPath, isLocal: true);
      ...
      if (_securityToken != null) {
        for (int i = 0; i < resultList.length; i++) {
          ...
          if (url != null && url.isNotEmpty) {
            _sendImg(
                _securityToken!.host + "/" + url, "${width},${height}", url);
          }
        }
        _images = [];
    ...
    if (pickedFile != null) {
      ...
      if (_securityToken != null) {
        ...
        final Directory _imageDirectory =
            await new Directory('${_directory.path}/activity/images/')
                .create(recursive: true);
        String _path = _imageDirectory.path;
        File imageFile = File('${_path}originalImage_$md5name.png')
        ...
        image.image.resolve(new ImageConfiguration()).addListener(
          ...
          String url = await _aliyunService.uploadImage(
              _securityToken!, imageFile.path, '${md5name}.png', _user.uid);
          if (url != null && url.isNotEmpty) {
            _sendImg(_securityToken!.host + "/" + url,
                "${info.image.width},${info.image.height}", url);
          }
        }));
      }
    }
  ...
  ///定位图片，位置名称
  Future<void> locationImage(Uint8List imgData, String address, String title,
      String latitude, String longitude) async {
    User _user = Global.profile.user!;
    _securityToken =
        await _aliyunService.getActivitySecurityToken(_user.token!, _user.uid);
    ...
    if (_securityToken != null) {
      ...
      image.image
        ...
        String url = await _aliyunService.uploadImage(
            _securityToken!, imageFile.path, '${md5name}.png', _user.uid);
        if (url != null && url.isNotEmpty) {
          _sendLocal(url, "${info.image.width},${info.image.height}", address,
              title, longitude, latitude, _securityToken!.host + "/" + url);
        }
      }));
    }
  }
            ...
            builder: (context, setState) {
              return Container(
                  padding: EdgeInsets.only(
                      left: 5, right: 5, top: _pagestatus + 10, bottom: 10),
                  color: Colors.white,
                  child: InkWell(
                    child: Column(
                      children: [
                        ...
                        Padding(
                          ...
                          child: TextField(
                            ...
                            onChanged: (value) {
                              ...
                              newArr = users
                                  .where((input) =>
                                      (input.username.indexOf(value) >= 0
                                          ? true
                                          : false))
                                  .toList();
                              setState(() {});
  ...
  Future<bool> _isQuitActivity() async {
    GroupRelation? temGroupRelation = await _imHelper.getGroupRelationByGroupid(
        Global.profile.user!.uid, _groupRelation.timeline_id);
    ...
    showDialog<Null>(
      ...
      builder: (BuildContext context) {
        return BlockPuzzleCaptchaPage(
          onSuccess: (v) {
            sendMessage(this._msgcontent, this._contenttype,
                this._localmsgcontent, this._localpath, v);
          },
          onFail: () {},
        );
      },
