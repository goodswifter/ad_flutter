...
class _CreateCommunityState extends State<CreateCommunity> {
  ...
  void _onLoading() async {
    if (!_ismore) return;
    final moredata =
        await _userService.getFans(Global.profile.user!.uid, users.length);
      ...
      if (imageFile != null) {
        ...
        if (croppedFile != null) {
          ...
          SecurityToken? securityToken =
              await _aliyunService.getActivitySecurityToken(
                  Global.profile.user!.token!, Global.profile.user!.uid);
          if (securityToken != null) {
            ossimagpath = await _aliyunService.uploadImage(securityToken,
                imageFile.path, '${md5name}.png', Global.profile.user!.uid);
          }
