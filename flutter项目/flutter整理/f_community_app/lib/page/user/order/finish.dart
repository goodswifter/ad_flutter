...
class _MyOrderFinishState extends State<MyOrderFinish> {
  ...
  _getMyOrder() async {
    _orderList = await _userService
        .getMyOrderFinish(Global.profile.user!.token!, Global.profile.user!.uid,
    ...
    return showDialog<Null>(
        ...
        builder: (BuildContext context) {
          return new AlertDialog(
            title: new Text('如果商家已经发货可能无法完成退款，确定要退货吗?',
  ...
  Future<void> _askedConfirm(String gpactid, String orderid) async {
    return showDialog<Null>(
        ...
        builder: (BuildContext context) {
          return new AlertDialog(
            title:
                new Text('确定已经收到商品了吗?', style: new TextStyle(fontSize: 17.0)),
            actions: <Widget>[
              new FlatButton(
                child: new Text('确定'),
                onPressed: () async {
  ...
  Future<void> _telCustomerCare(String vcode, int touid) async {
    ...
    if (groupRelation == null) {
      groupRelation = await _userService.joinSingleCustomer(
          timeline_id, uid, touid, Global.profile.user!.token!, vcode,
  ...
  //滑动拼图
  _loadingBlockPuzzle(BuildContext context,
      {barrierDismissible = true, required int touid}) {
    showDialog<Null>(
