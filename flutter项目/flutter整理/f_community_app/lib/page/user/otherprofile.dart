                  ...
                  child: Container(
                    ...
                    child: ClipOval(
                      child: Container(
                        ...
                        width: 30,
                        height: 30,
                        child: Icon(IconFont.icon_navbar_xiaoxi,
                            size: 19,
                            color: isBlack
                  ...
                  child: ClipOval(
                    child: Container(
                      ...
                      child: Icon(Icons.arrow_back_ios_new,
                          size: 19,
                          color:
                              isBlack ? textbarbackgroundColor : Colors.white),
                    ),
                  ),
                ),
                onTap: () {
                      ...
                      children: <Widget>[
                        ...
                        InkWell(
                          onTap: () {
                            if (user!.following! > 0) {
                              if (Global.profile.user != null) {
                                Navigator.pushNamed(context, '/OtherFollowUser',
                                    arguments: {"uid": user!.uid});
                              } else {
                                logIn();
                              }
  ...
  Widget buildCommunityInfo() {
    ...
    Widget btnJoin = Container(
      ...
      child: OutlineButton(
          onPressed: () {
            if (Global.profile.user != null) {
              Navigator.pushNamed(context, '/JoinFriend', arguments: {
...
//            _otherUserBloc.add(new JoinGroupMessage(Global.profile.user, Global.profile.user.mycommunity.cid));
  ...
  Future<void> showUserOperation() async {
    ...
    String blackName = "";
    List<int> blackList =
        await imHelper.getBlacklistUid(Global.profile.user!.uid);
                      ...
                      if (blackName == "解除黑名单") {
                        ...
                        if (ret) {
                          imHelper.delBlacklistUid(
                              Global.profile.user!.uid, user!.uid);
                        }
    ...
    return showDialog<Null>(
        ...
        builder: (BuildContext context) {
          return new AlertDialog(
            title: new Text('加入黑名单，他(她)将不能再给你发消息，并且不能参加你组织的活动',
  ...
  Future<void> joinMessage(String captchaVerification) async {
    ...
    if (groupRelation != null) {
      ...
      if (ret > 0) {
        Navigator.pushNamed(this.context, '/MyMessage',
  ...
  Future<void> followUser() async {
    bool ret = await _userService.Follow(Global.profile.user!.token!,
        Global.profile.user!.uid, user!.uid, errorCallBack);
    if (ret) {
  ...
  cancelFollow() async {
    bool ret = await _userService.cancelFollow(Global.profile.user!.token!,
        Global.profile.user!.uid, user!.uid, errorCallBack);
    if (ret) {
      await imHelper.delFollowState(user!.uid, Global.profile.user!.uid);
      Global.profile.user!.following = Global.profile.user!.following! - 1;
