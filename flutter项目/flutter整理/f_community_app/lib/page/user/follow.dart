...
class _MyFollowState extends State<MyFollow> {
  ...
  ScrollController _scrollControllerUserContent =
      ScrollController(initialScrollOffset: 0);
...
//        _lockUser = true;//加载完毕后再解锁
              ...
              builder: (context, state) {
                    ...
                    onTap: () {
                      Navigator.pushNamed(context, '/Login').then((value) {
                        if (Global.profile.user != null)
                          _myFollowBloc
                              .add(Refreshed(user: Global.profile.user!));
                      });
                    },
                  );
                }
                ...
                if (state is PostSuccess) {
                  return ListView(
                      ...
                      children:
                          buildContent(state.users, state.activitys, state));
                } else {
                  return Center(
                    child: CircularProgressIndicator(
                        valueColor:
  ...
  List<Widget> buildContent(
      List<User> users, List<Activity> activitys, MyFollowState state) {
...
//    );
  ...
  Widget activityContent(List<Activity> activitys, PostSuccess state) {
    ...
    return StaggeredGridView.countBuilder(
      ...
      itemCount:
          state.hasReachedActivityMax ? activitys.length : activitys.length + 1,
