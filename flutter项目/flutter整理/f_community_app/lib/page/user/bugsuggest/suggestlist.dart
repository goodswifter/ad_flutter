...
class _SuggestListState extends State<SuggestList> {
  ...
  void _getBugList() async {
    suggests = await _imService.getSuggestList(Global.profile.user!.uid,
  ...
  void _onLoading() async {
    if (!_ismore) return;
    final moredata = await _imService.getSuggestList(Global.profile.user!.uid,
        Global.profile.user!.token!, suggests.length, errorResponse);

    if (moredata.length > 0) suggests = suggests + moredata;
      ...
      suggests.forEach((e) {
        widgets.add(Card(
          ...
          child: Padding(
            ...
            child: InkWell(
              onTap: () {
                Navigator.pushNamed(context, '/SuggestInfo',
  ...
  Future<void> islike() async {
    for (int i = 0; i < suggests.length; i++) {
      await _imHelper.selBugAndSuggestState(
          suggests[i].suggestid, Global.profile.user!.uid, 1,
          (List<String> actid) {
        if (actid.length > 0) suggests[i].islike = true;
      });
    }
  }
