...
class _BugInfoState extends State<BugInfo> {
  ...
  getBug() async {
    _bug = await imService.getBugInfo(Global.profile.user!.uid,
        Global.profile.user!.token!, widget.bugid, errorCallBack);
    if (_bug != null) {
      ...
      await imhelper.selBugAndSuggestState(
          widget.bugid, Global.profile.user!.uid, 0, (List<String> actid) {
        ...
        children: <Widget>[
          ...
          Expanded(
            child: Column(
              ...
              children: <Widget>[
                Text(_bug!.user!.username,
                    style: TextStyle(
                        color: Colors.black87,
                        fontSize: 16,
                        fontWeight: FontWeight.bold)),
                Text(CommonUtil.datetimeFormat(
                    DateTime.parse(_bug!.createtime))),
              ],
            ),
          )
        ],
                              ...
                              IconButton(
                                ...
                                onPressed: () async {
                                  if (_isCommentLike) {
                                    ...
                                    if (v.likeuid == 0) {
                                      bool ret =
                                          await imService.updateBugCommentLike(
                                              v.commentid!,
                                      ...
                                      bool ret =
                                          await imService.delBugCommentLike(
                                              v.commentid!,
                                              Global.profile.user!.uid,
                                              Global.profile.user!.token!,
                                              v.user!.uid,
                            ...
                            PopupMenuItem(
                              child: InkWell(
                                ...
                                onTap: () async {
                                  ...
                                  if (ret) {
                                    listComments =
                                        await imService.getBugCommentList(
                                            widget.bugid,
                                            Global.profile.user!.uid,
                                            errorCallBack);
                    ...
                    onPressed: () async {
                      if (_message.isNotEmpty) {
                        if (commentid == 0) {
                          ...
                          Navigator.pop(context);
                          int temreplyid =
                              await imService.updateBugCommentReply(
                                  commentid,
                                  widget.bugid,
                                  Global.profile.user!.uid,
                                  Global.profile.user!.token!,
          ...
          return BlockPuzzleCaptchaPage(
            onSuccess: (v) async {
              if (commentid == 0) {
                ...
                listComments.insert(
                    0,
                    Comment(retcommentid, _bug!.bugid, Global.profile.user,
