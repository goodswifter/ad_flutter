...
class _BugListState extends State<BugList> {
  ...
  void _getBugList() async {
    bugs = await _imService.getBugList(Global.profile.user!.uid,
        Global.profile.user!.token!, 0, errorResponse);
    await islike();
  ...
  void _onLoading() async {
    if (!_ismore) return;
    final moredata = await _imService.getBugList(Global.profile.user!.uid,
        Global.profile.user!.token!, bugs.length, errorResponse);

    if (moredata.length > 0) bugs = bugs + moredata;
      ...
      bugs.forEach((e) {
        widgets.add(Card(
          ...
          child: Padding(
              ...
              child: InkWell(
                onTap: () {
                  Navigator.pushNamed(context, '/BugInfo',
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        IconText(
                          ...
                          onTap: () {
                            Navigator.pushNamed(context, '/BugInfo',
                                arguments: {"bugid": e.bugid}).then((val) {});
                          },
                        ),
                      ],
                    )
  ...
  Future<void> islike() async {
    for (int i = 0; i < bugs.length; i++) {
      await _imHelper.selBugAndSuggestState(
          bugs[i].bugid, Global.profile.user!.uid, 0, (List<String> actid) {
        if (actid.length > 0) bugs[i].islike = true;
      });
    }
  }
