...
class _MyAllReportListState extends State<MyAllReportList> {
  ...
  getMyReportList() async {
    myReports = await _activityService.getMyReport(
        Global.profile.user!.uid, Global.profile.user!.token!, (code, error) {
      ShowMessage.showToast(error);
    });
    if (myReports != null && myReports.length > 0) {
      myReports.forEach((e) {
        allReports.add(TemReport(
            reportid: e.reportid!,
            uid: e.uid!,
            actid: e.actid,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: pagestatus == 0
            ...
            : (myReports != null && myReports.length > 0
                ? buildContent()
                : Center(
                    child: Text(
                    '还没有举报记录',
    ...
    allReports.forEach((e) {
      if (e.type == 0) {
        ...
        lists.add(ListTile(
          ...
          onTap: () {
            Navigator.pushNamed(context, '/MyReportInfo', arguments: {
              "reportid": e.reportid,
              "sourcetype": e.sourcetype
            });
          },
      ...
      if (e.type == 1) {
        ...
        lists.add(ListTile(
          ...
          onTap: () {
            Navigator.pushNamed(context, '/MyReportImInfo',
