...
class _MyUserIdEditState extends State<MyUserId> {
                ...
                Card(
                  ...
                  child: Column(
                    children: [
                      ListTile(
                        onTap: () {
                          Navigator.pushNamed(context, '/MyUpdateMobile')
                              .then((value) {
                            setState(() {});
                          });
                        },
                      ...
                      ListTile(
                        onTap: () async {
                          if (Global.profile.user!.aliuserid == "") {
                            String authurl =
                                await _userService.getAliUserAuth();
                            //绑定支付宝账号
                                  ...
                                  if (_streamDemoSubscription == null) {
                                    _streamDemoSubscription =
                                      ...
                                      if (res is WeChatAuthResponse) {
                                        if (res.code != null) {
                                          User? user =
                                              await _userService.updateWeixin(
                                                  Global.profile.user!.uid,
                                                  Global.profile.user!.token!,
                                                  res.code!,
                                                  false,
  ...
  Future<void> _telCustomerCare(String vcode) async {
    ...
    customuid = await _commonJSONService.getSysCustomer(
        0, Global.profile.user!.uid, Global.profile.user!.token!);
    if (customuid <= 0) {
    ...
    GroupRelation? groupRelation =
        await _imhelper.getGroupRelationByGroupid(uid, timeline_id);
    if (groupRelation == null) {
      groupRelation = await _userService.joinSingleCustomer(
          timeline_id, uid, customuid, Global.profile.user!.token!, vcode,
          (String statusCode, String msg) {
        if (statusCode == "-1008") {
...
//                              flex: 7,
                            child: TextField(
                                ...
                                maxLength:
                                    15, //最大长度，设置此项会让TextField右下角有一个输入数量的统计字符串
                                maxLines: 1, //最大行数
                            ...
                            onPressed: () async {
                              if (_password1 == _password2) {
                                if (_password1.length >= 6) {
                                  BlocProvider.of<AuthenticationBloc>(context)
                                      .add(UpdateUserPasswordPressed(
                                          user: Global.profile.user!,
                                          password: _password1));
                                  Navigator.pop(context);
                                } else {
                                  ShowMessage.showToast(
                                      "新密码长度应在6-15位字符之间，请重新输入");
                                }
                              } else {
