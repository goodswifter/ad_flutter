...
class _MomentListState extends State<MomentList> {
  ...
  void _getMomentList() async {
    if (Global.profile.user != null) {
      moments = await _imService.getMomentList(
          0, Global.profile.user!.subject, _errorResponse);
      _notinteresteduids =
  ...
  void _onLoading() async {
    ...
    if (Global.profile.user != null) {
      ...
      moredata =
          await _imService.getMomentList(moments.length, "", _errorResponse);
    }

    if (moredata.length > 0) moments = moments + moredata;
  ...
  Future<void> _islike() async {
    for (int i = 0; i < moments.length; i++) {
      await _imHelper.selBugAndSuggestState(
          moments[i].momentid, Global.profile.user!.uid, 2,
          (List<String> actid) {
                ...
                Navigator.pushNamed(context, '/Login').then((value) {
                  if (Global.profile.user != null) {
                    Navigator.pushNamed(context, '/MomentReport').then((value) {
                      if (value != null && value != "") _getMomentList();
                    });
                  }
                });
              } else {
