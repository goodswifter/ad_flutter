...
class _MapLocationPickerState extends State<MapLocationPicker>
  ...
  final AMapPrivacyStatement aMapPrivacyStatement =
  ...
  void initState() {
    ...
    _kInitialPosition = CameraPosition(
        target: widget.latLng!, zoom: _currentZoom, tilt: 30, bearing: 0);
                          ...
                          onPressed: () async {
                            ...
                            if (widget.isMapImage) {
                              final Marker marker = Marker(
                                anchor: Offset(0.5, 1),
                                position: LatLng(_sendMsg!.latLng!.latitude,
                                    _sendMsg!.latLng!.longitude),
                                icon:
                                    BitmapDescriptor.fromIconPath(_indicator1),
  ...
  void _changeCameraPosition(LatLng latlng) {
    ...
    _controller?.moveCamera(
      CameraUpdate.newCameraPosition(
        CameraPosition(
            target: latlng, zoom: _currentZoom, tilt: 30, bearing: 0),
      ),
      animated: true,
    );
  }
    ...
    if (location != null &&
      ...
      _commonJSONService.getAmapPoi(
        ...
        poiList.forEach((e) {
          ...
          _poiInfoList.add(new MyAMapPoi(
              e["id"], e["name"], tem, e["address"], e["pcode"], e["adcode"]));
        });
        if (!ismore) {
  ...
  Future<void> _searchkeyword(String wordkeys, {bool ismore = false}) async {
    if (wordkeys != null && wordkeys.isNotEmpty) {
      ...
      _commonJSONService.getAmapWordKey(
          wordkeys, _searchtype, widget.citycode, true, 10, _page,
          (List poiList) {
        poiList.forEach((e) {
          List<double> tem = [];
          tem.add(double.parse(e['location'].toString().split(",")[1]));
