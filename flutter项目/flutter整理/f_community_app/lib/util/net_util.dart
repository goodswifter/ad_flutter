...
class NetUtil {
  ...
  Future<void> download(String url, String filepath, Function errorCallBack,
      ...
      if (response.data["status"] != null) {
        if (response.data["status"] < 0) {
          if (errorCallBack != null) {
            errorCallBack(response.data["status"].toString(),
  ...
  post(FormData formData, String api, Function callBack, Function errorCallBack,
      {bool isloginOut = false}) async {
    Response response;
    try {
  ...
  Future<void> wget(String url, Function callBack,
    ...
    if (response.statusCode == 200) {
      if (response.data["status"] == "1") callBack(response.data["pois"]);
    }
  }

  static Future<void> aliyunOSSpost(FormData formData, String url,
  ...
  static void _handError(Function errorCallback, Response response) {
    if (errorCallback != null) {
      errorCallback(
          response.data["status"].toString(), response.data["msg"].toString());
    }
