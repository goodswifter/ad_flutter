...
class NetworkManager {
  static final ImBloc _imBloc =
      BlocProvider.of<ImBloc>(Global.mainkey.currentContext!);
  ...
  static onDone({bool isouted = false}) async {
    if (channel != null) {
      if (Global.isInDebugMode) print("websocket done!");
      channel!.sink.close();
      channel = null;
  ...
  static onError(error) {
    if (Global.isInDebugMode) print(error);
  }
  ...
  static initLogin() async {
    if (Global.profile.user != null) {
      ...
      _imBloc.add(new UserRelationAndMessage(Global.profile.user!,
  ...
  static connect() {
    if (Global.profile.user != null) {
      if (Global.isInDebugMode) print("websocket init");
      channel = IOWebSocketChannel.connect('${Global.serviceIM}',
