...
class TokenUtil {
    ...
    connector.token.addListener(() {
      ...
      if (Platform.isIOS) {
        ...
        _userService.updatePushToken(
            ...
            Global.profile.user!.token!,
            "fcm",
            connector.token.value, (error, msg) {
          ShowMessage.showToast(msg);
        });
      }
    });
  ...
  deeplinkNav(String deeplink) async {
    ...
    if (deeplink.indexOf("timeline_id:") >= 0) {
      ...
      if (Global.profile.user != null) {
        GroupRelation? groupRelation = await _imHelper
            .getGroupRelationByGroupid(Global.profile.user!.uid, timeline_id);
        if (groupRelation != null) {
          Global.timeline_id = timeline_id;
          Navigator.pushNamed(Global.navigatorKey.currentContext!, '/MyMessage',
              arguments: {"GroupRelation": groupRelation}).then((val) {});
        }
      }
    }
    ...
    if (deeplink.indexOf("follow:") >= 0) {
      ...
      Navigator.pushNamed(Global.navigatorKey.currentContext!, '/OtherProfile',
          arguments: {"uid": uid});
    }
    ...
    if (deeplink.indexOf("bugid:") >= 0) {
      ...
      Navigator.pushNamed(Global.navigatorKey.currentContext!, '/BugInfo',
    ...
    if (deeplink.indexOf("suggestid:") >= 0) {
      ...
      Navigator.pushNamed(Global.navigatorKey.currentContext!, '/SuggestInfo',
    ...
    if (deeplink.indexOf("momentid:") >= 0) {
      ...
      Navigator.pushNamed(Global.navigatorKey.currentContext!, '/MomentInfo',
    ...
    if (deeplink.indexOf("evaluateid:") >= 0) {
      ...
      if (evaluateactivity != null) {
        Navigator.pushNamed(
            Global.navigatorKey.currentContext!, '/EvaluateInfo',
  ...
  Future<void> _gotoGoodPrice(String goodpriceid) async {
    ...
    if (goodprice != null) {
      Navigator.pushNamed(Global.navigatorKey.currentContext!, '/GoodPriceInfo',
