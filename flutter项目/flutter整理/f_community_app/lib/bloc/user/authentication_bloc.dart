import 'dart:async';
import 'package:f_community_app/service/userservice.dart';
import 'package:f_community_app/util/token_util.dart';
...
class AuthenticationBloc
  ...
  Stream<AuthenticationState> mapEventToState(
      ...
      try {
        ...
        if (user != null) {
          ...
          yield AuthenticationUnauthenticated(
              error: error, errorstatusCode: errorstatusCode);
        }
      } catch (error) {
        ///验证未通过
        yield AuthenticationUnauthenticated(
            error: errorNet, errorstatusCode: errorstatusCode);
      }
    }
    ...
    if (event is LoginWeiXin) {
      User? user =
          await _userService.loginweixin(event.auth_code, errorCallBack);
      if (user != null) {
        await LoginSuccess(user);
        yield AuthenticationAuthenticated();
      }
    ...
    if (event is LoginIos) {
      User? user = await _userService.loginIos(
          event.identityToken, event.iosuserid, errorCallBack);
      if (user != null) {
        await LoginSuccess(user);
        yield AuthenticationAuthenticated();
      }
    ...
    if (event is UpdateImagePressed) {
      try {
        bool ret = await userRepository.updateImage(
            event.user, event.serverimgpath, errorCallBack);
        if (ret) {
        ...
        ///验证未通过
        yield AuthenticationUnauthenticated(
            error: errorNet, errorstatusCode: errorNetCode);
      }
    }
    ...
    if (event is UpdateUserNamePressed) {
      try {
        bool ret = await userRepository.updateUserName(
            event.user, event.username, errorCallBack);
        if (ret) {
    ...
    if (event is UpdateUserSignaturePressed) {
      try {
        bool ret = await userRepository.UpdateUserSignaturePressed(
            event.user, event.signature, errorCallBack);
        if (ret) {
    ...
    if (event is UpdateUserLocationPressed) {
      try {
        bool ret = await userRepository.updateLocation(
            event.user, event.province, event.city, errorCallBack);
        if (ret) {
    ...
    if (event is UpdateUserPasswordPressed) {
      try {
        bool ret = await userRepository.UpdateUserPasswordPressed(
            event.user, event.password, errorCallBack);
        if (ret) {
          yield AuthenticationAuthenticated();
        } else {
          ///验证未通过
    ...
    if (event is UpdateUserSexPressed) {
      try {
        bool ret = await userRepository.UpdateUserSexPressed(
            event.user, event.sex, errorCallBack);
        if (ret) {
    ...
    if (event is UpdateUserBirthdayPressed) {
      try {
        bool ret = await userRepository.UpdateUserBirthdayPressed(
            event.user, event.birthday, errorCallBack);
        if (ret) {
    ...
    if (event is UpdateLocation) {
      yield UpdateLocationed(
          locationName: event.locationName, locationCode: event.locationCode);
    }
    //初始化用户信息更新
  ...
  Future<void> LoginSuccess(User user) async {
    ...
    if (user.likegoodpricecomment > 0) {
      activityService.getUserGoodPriceComnnentLike(user.uid, user.token!,
          user.likegoodpricecomment); //获取用户留言和评论的点赞情况goodprice
    }
    if (user.likebugcomment > 0 ||
      ...
      activityService.getUserBugAndSuggestAndMomentComnnentLike(
          ...
          user.likemomentcomment); //获取用户留言和评论的点赞情况comment和evaluate
    }
