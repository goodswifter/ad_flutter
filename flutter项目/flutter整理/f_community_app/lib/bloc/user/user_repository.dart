...
import 'package:f_community_app/util/common_util.dart';
...
class UserRepository {
  ...
  Future<User?> loginToUser(
    ...
    if (type == 1) {
      user = await _userService.login(
          mobile, password, captchaVerification, errorCallBack!);
    } else {
      user = await _userService.loginMobile(
          mobile, vcode, country, errorCallBack!);
    }
  ...
  Future<bool> updateImage(User user, String imgpath, Function errorCallBack) {
    return _userService.updateImageByUrl(
        user.token!, user.uid, imgpath, errorCallBack);
  }

  ///更新昵称
  Future<bool> updateUserName(
      User user, String username, Function errorCallBack) {
    return _userService.updateUserName(
        user.token!, user.uid, username, errorCallBack);
  }

  ///更新所在城市
  Future<bool> updateLocation(
      User user, String province, String city, Function errorCallBack) {
    return _userService.updateLocation(
        user.token!, user.uid, province, city, errorCallBack);
  }

  ///更新密码
  Future<bool> UpdateUserPasswordPressed(
      User user, String password, Function errorCallBack) {
    return _userService.updatePassword(
        user.token!, user.uid, password, errorCallBack);
  }

  ///更新兴趣
  Future<bool> UpdateUserInterest(
      User user, String interest, Function errorCallBack) {
    return _userService.updateInterest(
        user.token!, user.uid, interest, errorCallBack);
  }

  ///更新录音
  Future<bool> UpdateUserVoice(
      User user, String voice, Function errorCallBack) {
    return _userService.updateVoice(
        user.token!, user.uid, voice, errorCallBack);
  }

  ///更新性别
  Future<bool> UpdateUserSexPressed(
      User user, String sex, Function errorCallBack) {
    return _userService.updateSex(user.token!, user.uid, sex, errorCallBack);
  }

  ///更新生日
  Future<bool> UpdateUserBirthdayPressed(
      User user, String birthday, Function errorCallBack) {
    return _userService.updateBirthday(
        user.token!, user.uid, birthday, errorCallBack);
  }

  ///更新个人简介
  Future<bool> UpdateUserSignaturePressed(
      User user, String signature, Function errorCallBack) {
    return _userService.updateSignature(
        user.token!, user.uid, signature, errorCallBack);
  }
  ...
  Future<void> updateNotInteresteduids(
      User user, Function errorCallBack) async {
      ...
      if (notInteresteduids != null && notInteresteduids.length > 0) {
        for (int i = 0; i < notInteresteduids.length; i++) {
          imHelper.saveNotInteresteduids(
              user.uid, int.parse(notInteresteduids[i].toString()));
        }
      }
    }
  }
  ...
  Future<void> updateGoodPriceNotInteresteduids(
    ...
    if (ret.length <= 0) {
      List<String> goodpricenotinteresteduids =
          user.goodpricenotinteresteduids!.split(",");
      if (goodpricenotinteresteduids != null &&
          goodpricenotinteresteduids.length > 0) {
        for (int i = 0; i < goodpricenotinteresteduids.length; i++) {
          imHelper.saveGoodPriceNotInteresteduids(
              user.uid, int.parse(goodpricenotinteresteduids[i].toString()));
        }
      }
    }
  }
      ...
      if (blacklist != null && blacklist.length > 0) {
        for (int i = 0; i < blacklist.length; i++) {
          imHelper.saveBlacklistUid(
              user.uid, int.parse(blacklist[i].toString()));
        }
      }
    }
  }
  ...
  Future<bool> deleteToken(User user, Function errorCallBack) async {
    bool ret =
        await _userService.deltoken(user.token!, user.uid, errorCallBack);
    if (ret) {
  ...
  Future<void> updateUserPicture(User user, String profilepicture) async {
    ...
    Global.profile.defProfilePicture =
        await new NetworkImage(user.profilepicture!);
  ...
  successResponse(Map<String, dynamic> data) {
    if (data["data"]["token"].toString() != "") {
      user = User.fromJson(json
          .decode(CommonUtil.GetJsonString(data["data"]["token"].toString())));
