...
class ImBloc extends Bloc<ImEvent, ImState> {
  ...
  Stream<ImState> mapEventToState(
    ...
    try {
      if (event is UserRelationAndMessage) {
        ...
        if (event.userNotice != null) {
          ...
          if (event.userNotice!.unread_gpmsg > 0) {
            ...
            if (grouprelationlist != null && grouprelationlist.isNotEmpty) {
              await imService.saveLocalStore(grouprelationlist,
                  event.user.token!, event.user.uid, errorCallBack);
            }
          }
          ...
          //活动通知
          await activityService.saveLocalStore(event.userNotice!,
              event.user.token!, event.user.uid, errorCallBack);
        }
        //获取本地
        NewMessageState newMessageState =
            await setAppUnReadCount(event.user.uid);
        ...
        if (Global.timeline_id != "") {
          ...
          timeLineSyncs = await imHelper.getTimeLineSync(
              Global.profile.user!.uid, 0, 30, Global.timeline_id);
          Global.timeline_id = "";
      ...
      if (event is UserCommentReplyNotice) {
        ...
        if (userNotice != null) {
          await activityService.saveLocalStore(
              userNotice, event.user.token!, event.user.uid, errorCallBack);
        }
      }
        ...
        if (message != null && message != "") {
          ...
          if (await imHelper.saveGroupRelation(grouprelationlist) > 0) {
            ...
            await imService.saveTimeLineSync(
                timeLineSync, event.user.token, event.user.uid, errorCallBack);
          }
          timeLineSyncs = await imHelper.getTimeLineSync(
      ...
      if (event is ReCallMessage) {
        ...
        NewMessageState newMessageState =
            await setAppUnReadCount(event.user.uid);
        timeLineSyncs = await imHelper.getTimeLineSync(Global.profile.user!.uid,
            0, timeLineSyncs.length + 30, timeline_id);
        yield NewMessageState(
            sysMessage: newMessageState.sysMessage,
            groupRelations: newMessageState.groupRelations,
            msgMessage: timeLineSyncs);
  ...
  Future<NewMessageState> setAppUnReadCount(int uid) async {
    ...
    unAllReadCount = newMessageState.sysMessage.newImMode +
  ...
  Future<NewMessageState> getAllMessage(int uid) async {
    int unImReadCount = 0;
    List<GroupRelation> grouprelationlist =
        await imHelper.getGroupRelation(uid.toString());
    //IM为读
    ...
    int commentreply_count =
        await imHelper.getCommentReplysCount(); //新的留言,评论与回复
    int follow_count = await imHelper.getNewFollowCount(); //新的关注
    int activityevalute_count =
        await imHelper.getUserUnEvaluateOrder(); //有新的待评价活动不加入提醒，在用户主动点击我的页面后显示
